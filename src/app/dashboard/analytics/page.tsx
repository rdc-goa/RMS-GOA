...
import { Award, Download, Users, Loader2, FileArchive, Banknote, FileText, Calendar, Info, X, Clock } from 'lucide-react';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import { Button } from '@/components/ui/button';
import { useToast } from '@/hooks/use-toast';
import { toPng } from 'html-to-image';
import { useRouter } from 'next/navigation';
import { getStorageUsage, fetchAllClaimsAction, getTopCollaboratingInstitutesGemini } from '@/app/actions';
...
export default function AnalyticsPage() {
  ...
  const filteredEmrProjects = useMemo(() => {
    ...
    if (globalDateRange.start && globalDateRange.end) {
      ...
      const endDate = endOfMonth(parseISO(globalDateRange.end));
      base = base.filter(p => {
        const date = p.sanctionDate ? parseISO(p.sanctionDate) : (p.timestamp ? new Date(p.timestamp) : null);
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      await addImageToPdf(stage1ApproverChartRef, 'Stage 1 Approver Performance');

      doc.addPage(); yPos = 20;
      await addImageToPdf(waitTimeChartRef, 'Average Wait Time by Stage');

      doc.addPage(); yPos = 20;
      await addImageToPdf(submissionVsApprovalChartRef, 'Incentive Submissions vs. Approvals');

      doc.addPage(); yPos = 20;
      await addImageToPdf(collaborationChartRef, 'Research Collaboration Profile');
  ...
  useEffect(() => {
    ...
    if (isCro && user.faculties && user.faculties.length > 0) {
      ...
      projectsQuery = query(projectsCollection, where('institute', 'in', ['Parul College of Engineering', 'Parul College of Information Technology & Computer Science']));
      emrQuery = query(emrCollection, where('institute', 'in', ['Parul College of Engineering', 'Parul College of Information Technology & Computer Science']), where('status', 'in', ['Sanctioned', 'Process Complete']));
      claimsQuery = query(claimsCollection, where('faculty', '==', 'Faculty of Engineering, IT & CS'));
  ...
  const averageStageWaitTimesData = useMemo(() => {
    ...
    filteredIncentiveClaims.forEach(claim => {
      ...
      const s5 = claim.approvals?.find(a => a?.stage === 5);
      ...
      // Stage 5 Wait Time
      if (s4?.timestamp) {
        if (claim.status === 'Pending Stage 5 Approval') {
          const days = getDaysDifference(s4.timestamp, undefined);
          if (days !== null) stageTimes['Stage 5 Approver'].push(days);
        } else if (s5?.timestamp) {
          const days = getDaysDifference(s4.timestamp, s5.timestamp);
          if (days !== null) stageTimes['Stage 5 Approver'].push(days);
        }
      }
    });
  ...
  const submissionsVsApprovalsData = useMemo(() => {
    ...
    filteredIncentiveClaims.forEach(claim => {
      ...
      // 2. Process Approvals (fully approved ones)
      if (['Accepted', 'Submitted to Accounts', 'Payment Completed'].includes(claim.status)) {
        const lastApproval = Array.isArray(claim.approvals) 
          ? [...claim.approvals].filter(a => a?.status === 'Approved').sort((a, b) => b.stage - a.stage)[0]
          : null;
        const appTimestamp = lastApproval?.timestamp;
        if (appTimestamp) {
  ...
  const { totalImrGrantAmount, totalImrGrantsAwarded, totalEmrProjects } = useMemo(() => {
    ...
    const emrSanctionedCount = filteredEmrProjects.length;
    return {
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription>
                  Comparison of claims submitted vs. claims approved at Stage 3.
                </CardDescription>
              </div>
              <Button
                variant="outline"
            ...
            <CardContent>
              <div ref={submissionVsApprovalChartRef} className="p-4 bg-card">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription>
                  Share of publications and projects with external collaborators.
                </CardDescription>
              </div>
              <Button
                variant="outline"
                size="icon"
                onClick={() => handleExport(collaborationChartRef, 'research_collaboration_profile')}
                title="Export Chart"
              >
                <Download className="h-4 w-4" />
              </Button>
            ...
            <CardContent>
              <div ref={collaborationChartRef} className="p-4 bg-card">
                <ChartContainer config={collaborationConfig} className="h-[300px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <ChartTooltip
                        cursor={false}
