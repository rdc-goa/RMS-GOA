...
export default function BulkUploadPage() {
  ...
  const handleExport = async () => {
    ...
    const worksheet = workbook.addWorksheet("Bulk Upload History");
    
    worksheet.columns = [
      { header: 'Project ID', key: 'id', width: 25 },
      { header: 'Project Title', key: 'title', width: 40 },
      { header: 'PI Name', key: 'pi', width: 25 },
      { header: 'PI Email', key: 'pi_email', width: 30 },
      { header: 'Faculty', key: 'faculty', width: 20 },
      { header: 'Institute', key: 'institute', width: 20 },
      { header: 'Status', key: 'status', width: 15 },
      { header: 'Sanction Number', key: 'sanctionNumber', width: 20 },
      { header: 'Grant Amount', key: 'totalAmount', width: 15 },
    ...
    history.forEach(p => {
      worksheet.addRow({
        ...
        status: p.status,
        sanctionNumber: p.grant?.sanctionNumber || 'N/A',
        totalAmount: p.grant?.totalAmount || 0,
        sanctionDate: new Date(p.submissionDate).toLocaleDateString(),
      });
    });
    ...
    a.href = url;
    a.download = `bulk_upload_history_${new Date().toISOString().split('T')[0]}.xlsx`;
    a.click();
